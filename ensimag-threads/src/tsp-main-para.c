...
static void generate_tsp_jobs(struct tsp_queue *q, int hops, int len,
			      uint64_t vpres, tsp_path_t path,
	...
	if (hops == depth) {
		...
		for (int i = 0; i < nb_towns; i++) {
			if (!present(i, hops, path, vpres)) {
				...
				generate_tsp_jobs(q, hops + 1, len + dist,
						  vpres, path, cuts, sol,
						  sol_len, depth);
				vpres &= (~(1 << i));
			}
...
int main(int argc, char **argv)
{
	...
	pthread_t threads[nb_threads]; // Tableau de threads
	
	arguments *args = malloc(sizeof(arguments)); // Accès aux arguments pour les threads
    args->solution = &sol;
    args->solution_len = &sol_len;
    args->queue = &q;
    args->cuts = &cuts;
    args->vpres = vpres;
    
    for (int i = 0; i < nb_threads; i++) { // Création des threads
    	pthread_create(&threads[i], NULL, &tsp2, (void *)(args));
    }
		 
	void *status;
    for (int i = 0; i < nb_threads; i++) { // Terminaison des threads
	...
	printf
	    ("<!-- # = %d seed = %ld len = %d threads = %d time = %lld.%03lld ms ( %lld coupures ) -->\n",
	     nb_towns, myseed, sol_len, nb_threads, perf / 1000000ll,
	     perf % 1000000ll, cuts);
