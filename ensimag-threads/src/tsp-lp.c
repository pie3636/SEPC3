...
void save_lp(FILE * f, tsp_path_t path, int hops, int len, uint64_t vpres)
{
	...
	for (int i = 0; i < nb_towns; i++) {
		for (int j = 0; j < nb_towns; j++) {
			if (i != j && x[i][j]) {
				if (plus)
					fprintf(f, " +");
				fprintf(f, " %d x%d_%d", tsp_distance[i][j], i,
					j);
				plus = true;
			}
		}
	...
	for (int i = 0; i < nb_towns; i++) {
		if (!present(i, hops, path, vpres) || i == 0
		    || i == path[hops - 1]) {
			fprintf(f, "ville%d: ", i);
			bool plus = false;
			for (int j = 0; j < nb_towns; j++) {
...
int lower_bound_using_lp(tsp_path_t path, int hops, int len, uint64_t vpres)
{
	...
	if (SOLVEUR == SOLVEUR_NONE) {
		...
		case SOLVEUR_CBC:
			sol =
			    popen
			    ("cbc toto.lp | tee toto.sol | grep 'Objective value'",
			     "r");
		...
		case SOLVEUR_SYMPHONY:
			sol =
			    popen
			    ("symphony -L toto.lp | tee toto.sol | grep \"Solution Cost: \"",
			     "r");
		...
		case SOLVEUR_GLPSOL:
			sol =
			    popen
			    ("glpsol --lp toto.lp -o /dev/stdout | tee toto.sol | grep 'Objective:  L = '",
			     "r");
